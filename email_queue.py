...
class EmailQueue:
    ...
    @classmethod
    def enqueue(cls, to_email: str, email_type: EmailType, subject: str, body: str) -> bool:
        """
        将邮件加入发送队列
        ...
        try:
            ...
            with get_db() as db:
                db.execute(
                    """
                    INSERT INTO email_queue (to_email, email_type, subject, body, status, created_at)
                    VALUES (?, ?, ?, ?, 'pending', ?)
                    """,
                    (to_email, email_type.value, subject, body, now)
                )
                db.commit()
            return True
    ...
    @classmethod
    def get_pending(cls, limit: int = 10) -> list:
        """获取待发送的邮件"""
        try:
            with get_db() as db:
                rows = db.execute(
    ...
    def _worker_loop(cls) -> None:
        ...
        while cls._running:
            try:
                pending_emails = cls.get_pending(limit=5)
                ...
                for email in pending_emails:
                    ...
                    if success:
                        ...
                        print(f"[邮件队列] 发送成功: {email['to_email']} - {email['subject']}")
                    else:
                ...
                # 每处理一批后清理旧邮件
                if datetime.now().minute == 0:  # 每小时执行一次
...
"""
    return EmailQueue.enqueue(to_email, EmailType.VERIFICATION, subject, body)
...
"""
    return EmailQueue.enqueue(to_email, EmailType.REGISTER_SUCCESS, subject, body)
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="alert">
                ...
                <p><strong>登录IP：</strong>{ip_address}</p>
            </div>
...
"""
    return EmailQueue.enqueue(to_email, EmailType.LOGIN_ALERT, subject, body)
